...
signal score_updated(score: int, level: int, lines: int)
signal game_over
...
func clear_board_pieces(board_pieces):
	...
	if lines_cleared > 0:
		process_score(lines_cleared)

func process_score(lines_cleared: int):
	...
	score_updated.emit(score, level, lines_cleared_total)
...
func move_all_row_pieces_down(board_pieces, cleared_row_number):
	for i in range(cleared_row_number - 1, 0, -1):
